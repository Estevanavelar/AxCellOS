...
# <=== Inicio STEP00 ===>
    ...
    # <=== Inicio SET Diretorios ===>
        ...
        dirDefault=/opt/InstaladorAXCELL
...
# <=== Inicio Termos de Aceite ===>
    ...
    echo "Recomendamos que, se houver instalações anteriores do AxCell-OS, faça backup de todos os dados importantes antes de prosseguir com a instalacao. Ao continuar com a instalacao, você concorda que nao responsabilizara os desenvolvedores do script por quaisquer perdas de dados que possam ocorrer."
    echo
...
#  <=== Inicio STEP03 ===>
    ...
    echo "03.1 Verificando AxCell-OS GitHUB"
    if [ -d "$dirHtdocs/axcell" ]
    then
        echo "* AxCell-OS presente no sistema."
    else
        echo "* Baixando a ultima versao do projeto."
        wget --quiet --show-progress -O $dirDefault/AxCell-OS.zip $(curl -s https://api.github.com/repos/RamonSilva20/axcell/releases/latest | grep "zipball_url" | awk -F\" '{print $4}')
        echo
        echo "* Extraindo projeto."
        unzip -q $dirDefault/AxCell-OS.zip -d $dirHtdocs/
        mv -i $dirHtdocs/RamonSilva20-axcell-* $dirHtdocs/axcell
        echo
        echo "* Atribuindo permissões."
        sudo chmod 777 $dirHtdocs/axcell/updates/
        sudo chmod 777 $dirHtdocs/axcell/index.php
        sudo chmod 777 $dirHtdocs/axcell/application/config/config.php
        sudo chmod 777 $dirHtdocs/axcell/application/config/database.php
        echo
        echo "* Criando banco de dados."
        $dirMySQL -u root -e "CREATE DATABASE axcell;"
    fi
# <=== Fim STEP03 ===>
...
# <=== Inicio STEP04 ===>
    ...
    if [ -f "$dirHtdocs/axcell/application/vendor" ]
    then
...
# <=== Inicio STEP05 ===>
    ...
    echo
    echo "05.1 Configurar AxCell-OS (Browser)"
    echo "Acesse o AxCell-OS via Browser"
    ...
    echo "URL: http://localhost/axcell (ou IP/Dominio)"
...
# <=== Inicio STEP06 ===>
    ...
    # <=== Inicio Configuracao de Email ===>
        ...
        if [ "$resposta" = "N" ] || [ "$resposta" = "n" ]; then
            ...
            sed -i "s/\EMAIL_PROTOCOL.*/\EMAIL_PROTOCOL='$protocolo'/" $dirHtdocs/axcell/application/.env
            sed -i "s/\EMAIL_SMTP_HOST.*/\EMAIL_SMTP_HOST='$hostsmtp'/" $dirHtdocs/axcell/application/.env
            sed -i "s/\EMAIL_SMTP_CRYPTO.*/\EMAIL_SMTP_CRYPTO='$criptografia'/" $dirHtdocs/axcell/application/.env
            sed -i "s/\EMAIL_SMTP_PORT.*/\EMAIL_SMTP_PORT='$porta'/" $dirHtdocs/axcell/application/.env
            sed -i "s/\EMAIL_SMTP_USER.*/\EMAIL_SMTP_USER='$email'/" $dirHtdocs/axcell/application/.env
            sed -i "s/\EMAIL_SMTP_PASS.*/\EMAIL_SMTP_PASS='$senha'/" $dirHtdocs/axcell/application/.env
            echo
    ...
    # <=== Inicio Configuracao da Cron ===>
        ...
        if [ "$resposta" = "N" ] || [ "$resposta" = "n" ]; then
            ...
            (crontab -l ; echo "*/2 * * * * php $dirHtdocs/axcell/index.php email/process") | crontab -
            (crontab -l ; echo "*/5 * * * * php $dirHtdocs/axcell/index.php email/retry") | crontab -
        fi
    ...
    if [ "$resposta" = "N" ] || [ "$resposta" = "n" ]; then
        ...
        $dirMySQL -u root -e "USE axcell; ALTER TABLE os AUTO_INCREMENT=$nOS;"
