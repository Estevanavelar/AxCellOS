...
class Login extends CI_Controller
{
    ...
    public function verificarLogin()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->model('Axcell_model');
            $user = $this->Axcell_model->check_credentials($email);
            ...
            if ($user) {
                ...
                if (password_verify($password, $user->senha)) {
                    ...
                    $json = ['result' => false, 'message' => 'Os dados de acesso estão incorretos.', 'AXCELL_TOKEN' => $this->security->get_csrf_hash()];
                    echo json_encode($json);
                }
            } else {
                $json = ['result' => false, 'message' => 'Usuário não encontrado, verifique se suas credenciais estão corretass.', 'AXCELL_TOKEN' => $this->security->get_csrf_hash()];
                echo json_encode($json);
            }
        }
        die();
