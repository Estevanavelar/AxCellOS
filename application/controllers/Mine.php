...
class Mine extends CI_Controller
{
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($cliente) {
                ...
                if (password_verify($password, $cliente->senha)) {
                    ...
                    echo json_encode(['result' => false, 'message' => 'Os dados de acesso estão incorretos.', 'AXCELL_TOKEN' => $this->security->get_csrf_hash()]);
                }
            } else {
                echo json_encode(['result' => false, 'message' => 'Usuário não encontrado, verifique se suas credenciais estão corretass.', 'AXCELL_TOKEN' => $this->security->get_csrf_hash()]);
            }
        }
    }
    ...
    public function visualizarOs($id = null)
    {
        ...
        $data['emitente'] = $this->axcell_model->getEmitente();
...
/* Location: ./application/controllers/conecte.php */
    ...
    public function catalogo()
    {
        ...
        $data['produtos'] = $this->produtos_model->get('produtos');
        $data['servicos'] = $this->servicos_model->get('servicos');
        $data['menuCatalogo'] = 'Catálogo';
        $data['tituloMenuCatalogo'] = 'Catálogo de Produtos e Serviços';
        $data['output'] = 'conecte/catalogo';
