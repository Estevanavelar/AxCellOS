...
class Axcell extends MY_Controller
{
    /**
     * author: Avelar Company
    ...
    {
        parent::__construct();
        $this->load->model('axcell_model');
    }

    public function index()
    {
        $this->data['ordens'] = $this->axcell_model->getOsAbertas();
        $this->data['ordens1'] = $this->axcell_model->getOsAguardandoPecas();
        $this->data['ordens_andamento'] = $this->axcell_model->getOsAndamento();
        $this->data['produtos'] = $this->axcell_model->getProdutosMinimo();
        $this->data['os'] = $this->axcell_model->getOsEstatisticas();
        $this->data['estatisticas_financeiro'] = $this->axcell_model->getEstatisticasFinanceiro();
        $this->data['financeiro_mes_dia'] = $this->axcell_model->getEstatisticasFinanceiroDia($this->input->get('year'));
        $this->data['financeiro_mes'] = $this->axcell_model->getEstatisticasFinanceiroMes($this->input->get('year'));
        $this->data['financeiro_mesinadipl'] = $this->axcell_model->getEstatisticasFinanceiroMesInadimplencia($this->input->get('year'));
        $this->data['menuPainel'] = 'Painel';
        $this->data['view'] = 'axcell/painel';
        return $this->layout();
    }

    public function minhaConta()
    {
        $this->data['usuario'] = $this->axcell_model->getById($this->session->userdata('id_admin'));
        $this->data['view'] = 'axcell/minhaConta';
        return $this->layout();
    }

    public function alterarSenha()
    {
        $current_user = $this->axcell_model->getById($this->session->userdata('id_admin'));

        if (!$current_user) {
            $this->session->set_flashdata('error', 'Ocorreu um erro ao pesquisar usuário!');
            redirect(site_url('axcell/minhaConta'));
        }
        ...
        $result = $this->axcell_model->alterarSenha($senha);
    ...
    public function pesquisar()
    {
        ...
        $data['results'] = $this->axcell_model->pesquisar($termo);
        $this->data['produtos'] = $data['results']['produtos'];
        $this->data['servicos'] = $data['results']['servicos'];
        $this->data['os'] = $data['results']['os'];
        $this->data['clientes'] = $data['results']['clientes'];
        $this->data['view'] = 'axcell/pesquisa';
        return $this->layout();
    }
    ...
    public function emitente()
    {
        ...
        $this->data['dados'] = $this->axcell_model->getEmitente();
        $this->data['view'] = 'axcell/emitente';
        return $this->layout();
    }
    ...
    public function cadastrarEmitente()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            redirect(site_url('axcell/emitente'));
        } else {
            ...
            $retorno = $this->axcell_model->addEmitente($nome, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email, $logo);
            if ($retorno) {
    ...
    public function editarEmitente()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $retorno = $this->axcell_model->editEmitente($id, $nome, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email);
            if ($retorno) {
    ...
    public function editarLogo()
    {
        ...
        $retorno = $this->axcell_model->editLogo($id, $logo);
        if ($retorno) {
            $this->session->set_flashdata('success', 'As informações foram alteradas com sucesso.');
    ...
    public function uploadUserImage()
    {
        ...
        $usuario = $this->axcell_model->getById($id);
        ...
        $imageUserPath = $image;
        $retorno = $this->axcell_model->editImageUser($id, $imageUserPath);
    ...
    public function emails()
    {
        ...
        $this->data['configuration']['base_url'] = site_url('axcell/emails/');
    ...
    public function excluirEmail()
    {
        ...
        if ($id == null) {
            ...
            redirect(site_url('axcell/emails/'));
        }
    ...
    public function atualizarBanco()
    {
        ...
        return redirect(site_url('axcell/configurar'));
    }

    public function atualizarAxcell()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'cSistema')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para configurar o sistema');
            redirect(base_url());
        ...
        if (!$this->github_updater->has_update()) {
            $this->session->set_flashdata('success', 'Seu axcell já está atualizado!');
        ...
        if ($success) {
            $this->session->set_flashdata('success', 'Axcell atualizado com sucesso!');
        } else {
            $this->session->set_flashdata('error', 'Erro ao atualizar axcell!');
        }
    ...
    public function calendario()
    {
        ...
        $allOs = $this->axcell_model->calendario(
            $start,
            $end,
            $status
        );
