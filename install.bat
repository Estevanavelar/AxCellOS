TITLE Instalador AxCell-OS Windows 10/11
@ECHO OFF
CLS
ECHO =============================
...
:: Publicado na versao 4.41.0 do AxCell-OS
::::::::::::::::::::::::::::
...
ECHO  **    AXCELL-OS - SISTEMA DE ORDEM DE SERVICO      **
ECHO  **             Windows 10/11                    **
ECHO  **                                              **
ECHO  **                                              **
ECHO  **                                              **
...
SET dirDefault=C:\InstaladorAXCELL
...
ECHO 01.5 Verificando AxCell-OS GitHUB
IF not EXIST "%dirDefault%\AxCell-OS.zip" FOR /F "eol= tokens=2 delims=, " %%A IN (' cURL -s https://api.github.com/repos/RamonSilva20/axcell/releases/latest ^| findstr /I /C:"zipball_url" ') DO PowerShell -command "& { iwr %%A -OutFile %dirDefault%\AxCell-OS.zip }"
SET stepnext=step02
GOTO step00
:: <=== Fim STEP01 ===>
...
:: <=== Inicio STEP03 ===>
:step03
ECHO 03 INSTALACAO SISTEMA AXCELL-OS...
ECHO.
ECHO 03.1 Extracao dos arquivos AXCELL-OS
IF EXIST %dirHtdocs%\axcell\index.php SET stepnext=step04 && GOTO step00
PowerShell -ExecutionPolicy Bypass -Command "Expand-Archive %dirDefault%\AxCell-OS.zip %dirHtdocs%" -Force
IF %ErrorLevel% GTR 0 ( DEL %dirDefault%\AxCell-OS.zip && ECHO Falha na extracao do AxCell-OS, efetuando novo download. && SET stepnext=step01 && GOTO step00 )
ECHO 03.2 Correcao da Pasta AXCELL-OS
FOR /F "tokens=4" %%B IN ( ' dir "%dirHtdocs%\" ^| findstr /I /C:"RamonSilva20" ' ) DO IF NOT EXIST %dirHtdocs%\axcell rename %dirHtdocs%\%%B axcell
SET stepnext=step04
GOTO step00
:: <=== Fim STEP03 ===>
...
IF NOT EXIST %dirHtdocs%\axcell\application\vendor START /I /D %dirHtdocs%\axcell /WAIT PowerShell C:\ProgramData\ComposerSetup\bin\composer install --no-dev
SET stepnext=step05
GOTO step00
:: <=== Fim STEP04 ===>
...
:: <=== Inicio STEP05 ===>
:step05
ECHO 05 CONFIGURANDO AXCELL...
ECHO.
ECHO 05.1 Criar Banco de Dados (axcell)
%dirMySQL%\mysql.exe -u "root" -e "create database `axcell`;" >NUL 2>&1
ECHO 05.2 Configurar AxCell-OS (Browser)
ECHO 05.2.1 Insira as configuracoes abaixo:
ECHO.
ECHO Host: localhost
ECHO Usuario: root
ECHO Senha: "Em Branco"
ECHO Banco de Dados: axcell
ECHO URL: http://localhost/axcell
start /B http://localhost/axcell
ECHO.
CHOICE /C SN /M "Configuracao do AxCell-OS Finalizada?"
...
IF ERRORLEVEL 1 SET dirEmail=%dirHtdocs%\axcell\application\.env
...
ECHO $action = New-ScheduledTaskAction 'C:\xampp\php\php.exe' -Argument 'index.php email/process' -WorkingDirectory 'C:\xampp\htdocs\axcell'>%ps%
ECHO $trigger = New-ScheduledTaskTrigger -AtStartup>>%ps%
ECHO $task = Register-ScheduledTask -TaskName "AxcellEnvioEmail" -Description "Comando responsável por verificar e disparar os e-mails pendentes no sistema Axcell. Criado por Bruno Barreto e Leonardo Bernardi" -Trigger $trigger -Action $action -RunLevel Highest>>%ps%
ECHO $task.Triggers.Repetition.Interval= 'PT2M'>>%ps%
PowerShell -command "&Add-Content -Path '%ps%' -Value '$task | Set-ScheduledTask'"
ECHO $action = New-ScheduledTaskAction 'C:\xampp\php\php.exe' -Argument 'index.php email/retry' -WorkingDirectory 'C:\xampp\htdocs\axcell'>>%ps%
ECHO $trigger = New-ScheduledTaskTrigger -AtStartup>>%ps%
ECHO $task = Register-ScheduledTask -TaskName "AxcellReenvioEmail" -Description "Comando responsável por verificar e disparar os e-mails pendentes no sistema Axcell. Criado por Bruno Barreto e Leonardo Bernardi" -Trigger $trigger -Action $action -RunLevel Highest>>%ps%
...
%dirMySQL%\mysql.exe -u "root" -e "use axcell; ALTER TABLE os AUTO_INCREMENT=%nOS%;" >NUL 2>&1
...
:stepfim
ECHO  ************************************************
ECHO  ****    AXCELL CONFIGURADO COM SUCESSO       ****
ECHO  ************************************************
GOTO StepSair
:: <=== Inicio STEP FIM ===>
